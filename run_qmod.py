...
def main():
    ...
    print("=" * 60)
    print("Quantum-Modulated DLinear (QMod)")
    print("=" * 60)

    save_dir = os.path.join(BASE_DIR, "qmod")
    ...
    run = wandb.init(
        ...
        name="QMod",
        config=run_config,
        tags=["quantum", "qmod", "novel"],
        reinit=True,
    )

    trainer = Trainer(model, run_config, save_dir, run_name="qmod")
    ...
    art = wandb.Artifact("qmod_best", type="model")
    ...
    # log learned modulation pattern
    model.eval()
    all_mods = []
    with torch.no_grad():
        for bx, _ in test_loader:
            seasonal, _ = model.decomp(bx)
            B = bx.shape[0]
            ctx = model.time_adapter(seasonal.permute(0, 2, 1))
            q_in = model.feature_compressor(ctx.reshape(B, -1)) * np.pi
    ...
    ax.plot(hours, mean_mod, 'o-', color='#8172B3', markersize=5)
    ax.fill_between(hours, mean_mod - std_mod, mean_mod + std_mod,
                    color='#8172B3', alpha=0.2)
    ax.axhline(0, color='gray', linestyle='--', linewidth=0.8)
    ax.set_xlabel('Forecast Hour')
    ax.set_ylabel('Modulation Factor')
    ax.set_title('QMod — Learned Per-Hour Quantum Modulation')
    ax.set_xticks(hours)
    ...
    print(f"\nMean modulation per hour:")
    for h, m in zip(hours, mean_mod):
        print(f"  Hour {h:2d}: {m:+.4f}")
